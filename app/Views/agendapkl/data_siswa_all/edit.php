	...
	<section class="section">
		<div class="card">
			<form action="<?= base_url('agendapkl/data_siswa_all/aksi_edit/')?>" method="post" class="row g-3" enctype="multipart/form-data">
				...
				<input type="hidden" name="id2" value="<?php echo $jojo->user_siswa ?>">
					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							<div class="mb-3">
								...
								<input type="text" class="form-control" id="telpon" placeholder="Masukkan Telpon" name="telpon" value="<?php echo $jojo->telpon_siswa ?>" required>
							</div>
							<div class="mb-3">
								<label for="namasiswa" class="form-label">Jurusan</label>
								<select class="form-select" id="jurusan" placeholder="Masukkan Jenis Kelamin" name="jurusan" value="<?php echo $jojo->jurusan ?>" required>
							...
							<div class="mb-3">
								...
								<input type="text" class="form-control" id="nama_pt" placeholder="Masukkan Nama PT" name="nama_pt" value="<?php echo $jojo->nama_pt ?>" required>
							</div>
							<div class="mb-3">
								<label for="namasiswa" class="form-label">Guru Pembimbing</label>
								<select class="form-select" id="guru" placeholder="Masukkan Guru" name="guru_pembimbing" value="<?php echo $jojo->guru_pembimbing ?>" required>
									<option value="<?php echo $jojo->guru_pembimbing ?>">- Pilih -</option>
									<?php 
									foreach ($guru as $pembimbing) {
										?>
										<option value="<?=$pembimbing->user_guru?>"><?= $pembimbing->nama_guru?></option>								
									<?php } ?>
								</select>
							</div>
							<div class="mb-3">
								...
								<select class="form-select" id="instruktur" placeholder="Masukkan Instruktur" name="instruktur" value="<?php echo $jojo->instruktur ?>" required>
									...
									foreach ($instruktur as $pt) {
										?>
										<option value="<?=$pt->user_instruktur?>"><?= $pt->nama_instruktur?></option>
									<?php } ?>
								</select>
							</div>
							<!-- form bagian kanan -->
